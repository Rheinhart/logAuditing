        ...
        try:
            ...
            for line in lines:
                ...
                if WARN_SUCCESS in line[3] or INFO_SUCCESS in line[3]:# 判断是否为log球队信息行: info 下注成功, warn 补单
                    ...
                    if 'WARN' in line[2]:
                        self.income = re.findall("\xca\xd5\xc8\xeb(.*)", line[4])[0]  # parser 收入
                        #self.info =u'补单成功'
                    elif 'INFO' in line[2]:
                        self.income = re.findall("\xca\xd5\xc8\xeb:(.*)", line[4])[0]  # parser 收入
                        #self.info =u'下注成功'

                    game = re.findall("\xb6\xd3\xce\xe9:(.*)", line[5])[0]             # parser 主队..and
                    games = ''
                    for i in range(6, len(line)-4):                                    # 合并比赛队伍到一个value
                        games +=' '+str(line[i])
                    self.game = game+games
                    self.prelevel = re.findall("\xd4\xa4\xc6\xda\xcb\xae\xce\xbb:(.*)", line[-4])[0]  # parser 预期水位
                    self.big = line[-2]  #parser 大或让球

...
class PinnacleCheck(object):
    ...
    def __login_main(self):
        ...
        postData = {
            '__VIEWSTATE': '/wEPDwUINDc3MzExNzIPZBYCAgMPZBYCAgEPZBYCAgMPEGQPFgECBxYBEAUEVGhhaQUCdGhnFgFmZGRevYPRLil7tSWwrGv94kOQWgS/Z/UdYWX+2Psw3JdwrQ==',
    ...
    def __find_ticket(self, ticket_No):
        ...
        try:
            ...
            if isticket:
                ...
                hasSuccess = re.findall(tagWin, ticketinfo.content) or re.findall(tagLoss,
                                                                                  ticketinfo.content) or re.findall(
                    tagPending, ticketinfo.content)
    ...
    def ticket_check(self, nickname, ticket):
        ...
        try:
            return self.__find_ticket(ticket)
        except:
            return False


class ZhiboCheck(object):
    ...
    finduserpage_no_permission = 'http://isn999.com/managersite/betList/statementBL?idx=%today%&winid=betList'
...
class SboCheck(object):
    ...
    def ticket_check(self, nickname, ticket):
        ...
        try:
            return self.__find_ticket(nickname, ticket)
        except Exception, e:
            print e
            return False
...
def load_account(cfile):
    ...
    try:
        ...
        config.read(cfile)
        user_list['Pinnacle_username'] = config.get('Pinnacle', 'Username')
        user_list['Pinnacle_password'] = config.get('Pinnacle', 'Password')
        user_list['Zhibo_username'] = config.get('Zhibo', 'Username')
        user_list['Zhibo_password'] = config.get('Zhibo', 'Password')
        user_list['Sbo_username'] = config.get('Sbo', 'Username')
        user_list['Sbo_password'] = config.get('Sbo', 'Password')
        return user_list
